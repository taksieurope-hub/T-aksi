...
import { useAuth, API_BASE, GOOGLE_MAPS_API_KEY } from "@/config";
...
// --- MAP PICKER (FIXED: resize + cleanup) ---
const MapPicker = ({ isOpen, onClose, onLocationSelect, title, initialLocation, mapsLoaded }) => {
  ...
  const clearListeners = () => {
    listenersRef.current.forEach((l) => l && window.google?.maps?.event?.removeListener(l));
  ...
  useEffect(() => {
    ...
    // When opening: reset previous state
    ...
    listenersRef.current.push(map.addListener("click", (e) => updateLocation(e.latLng)));
    listenersRef.current.push(marker.addListener("dragend", () => updateLocation(marker.getPosition())));

    // IMPORTANT FIX: when dialog opens, map container often starts with wrong size -> force resize
    setTimeout(() => {
  ...
  }, [isOpen, mapsLoaded, initialLocation, reverseGeocode]);
  ...
  const handleConfirm = () => {
    if (!selectedLocation) return toast.error("Please select a location on the map");
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-black border border-[#00ff88]/30 w-[95vw] max-w-md h-[90vh] flex flex-col p-0 gap-0">
        ...
        <DialogHeader className="p-4 bg-black/80 z-10 w-full border-b border-[#00ff88]/20 flex-none">
          ...
          <DialogDescription className="text-gray-500 text-xs">Tap map / drag pin.</DialogDescription>
        </DialogHeader>
        ...
        <div className="w-full p-4 bg-black border-t border-[#00ff88]/30 flex flex-col gap-3 flex-none">
          ...
          <div className="flex gap-2">
            <Button variant="outline" className="border-[#00d4ff]/30 text-[#00d4ff] flex-1" onClick={getCurrentLocation} disabled={loading}>
              {loading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Crosshair className="w-4 h-4 mr-2" />}
              {t("gps_btn")}
            </Button>
            <Button className="flex-1 bg-[#00ff88] text-black font-bold" onClick={handleConfirm} disabled={!selectedLocation}>
              {t("confirm_location")}
            </Button>
          </div>
        </div>
...
// --- LOCATION INPUT (FIXED: controlled input) ---
  ...
  return (
    <>
      <div className="relative flex items-center mb-2">
        ...
        />
        <Button type="button" variant="ghost" size="icon" className="absolute right-1 text-[#00d4ff] z-20 hover:bg-[#00d4ff]/10" onClick={() => setShowMapPicker(true)}>
...
// --- LIVE TRACKING MAP (FIXED: init when mapsLoaded becomes true) ---
const LiveTrackingMap = ({ pickup, destination, driverLocation, status, mapsLoaded }) => {
  const mapRef = useRef(null);
  const mapInstanceRef = useRef(null);
  const rendererRef = useRef(null);
  const [eta, setEta] = useState(null);

  // init map when mapsLoaded is ready (NOT only once on mount)
  useEffect(() => {
  ...
  // draw route / update ETA
  useEffect(() => {
    if (!mapsLoaded || !window.google || !rendererRef.current) return;
  ...
  }, [mapsLoaded, driverLocation, status, pickup, destination]);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isLogin && (
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="bg-black/50 border-[#00ff88]/30 text-white" required />
                </div>
                <div className="space-y-2">
                  <Label className="text-[#00ff88]">{t("last_name")}</Label>
                  <Input value={formData.surname} onChange={(e) => setFormData({ ...formData, surname: e.target.value })} className="bg-black/50 border-[#00ff88]/30 text-white" required />
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input type="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} className="pl-10 bg-black/50 border-[#00ff88]/30 text-white" required />
              </div>
            </div>
...
const RiderDashboard = () => {
  ...
  const calculateRoute = useCallback(async () => {
    if (!mapsLoaded || !window.google || !pickup.lat || !destination.lat) return;
    const directionsService = new window.google.maps.DirectionsService();
  ...
  useEffect(() => {
    if (pickup.lat && destination.lat) calculateRoute();
  }, [pickup, destination, stops, calculateRoute]);
  ...
  const addStop = () => {
    if (stops.length < 3) setStops([...stops, { address: "", lat: null, lng: null, order: stops.length }]);
    else toast.error("Maximum 3 stops allowed");
  };
  ...
  const handleBookRide = async (paid = false, paymentOrderId = null) => {
    if (!pickup.lat || !pickup.address) return toast.error("Please select pickup location");
      ...
      <main className="container mx-auto p-4 max-w-2xl">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {carTypes.map((type) => (
                    <button
                      ...
                      className={`p-2 rounded-xl border transition-all flex flex-col items-center ${carType === type.value ? "border-[#00ff88] bg-[#00ff88]/20" : "border-gray-700 bg-black"}`}
                    >
                      <span className="text-xl">{type.icon}</span>
                      <span className="text-white text-xs mt-1">{type.label}</span>
                      <span className="text-[#00ff88] text-xs font-bold mt-1">
                        {routeInfo ? `₾${calculateFare(type.value, routeInfo.distance, 0, 0, 0, surgeInfo?.multiplier || 1.0).total.toFixed(0)}` : `₾${type.base}`}
                      </span>
                    </button>
                  ))}
                </div>
                    ...
                    <PayPalButtons
                      ...
                      createOrder={async (_, actions) =>
                        actions.order.create({
                          purchase_units: [{ amount: { value: String(fareEstimate.total), currency_code: "USD" } }],
                        })
                      }
              ...
              <Card className="bg-black/60 backdrop-blur-xl border border-[#00d4ff]/30 overflow-hidden">
                {["accepted", "arrived", "in_progress"].includes(activeRide.status) && (
                  <LiveTrackingMap
                    ...
                    destination={{
                      lat: parseFloat(activeRide.destinationLat || 0),
                      lng: parseFloat(activeRide.destinationLng || 0),
                    }}
                    driverLocation={{
                ...
                <CardContent className="space-y-4 pt-4">
                  <div className="flex justify-between items-center mb-2">
                    <Badge className={`${statusColors[activeRide.status]} text-sm px-3 py-1`}>{activeRide.status?.replace(/_/g, " ").toUpperCase()}</Badge>
                    ...
                    <div className="bg-[#1a1a2e] rounded-xl p-4 border border-[#00d4ff]/20 shadow-lg relative overflow-hidden">
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        ...
                        <div className="flex flex-col gap-2">
                          {activeRide.driver_info?.cellphone ? (
                            <a href={`tel:${activeRide.driver_info.cellphone}`}>
                              <Button size="icon" className="rounded-full bg-[#00ff88] text-black hover:bg-[#00ff88]/80">
                                <Phone size={18} />
                              </Button>
                            </a>
                          ) : (
                            <Button size="icon" className="rounded-full bg-[#00ff88] text-black hover:bg-[#00ff88]/80" disabled>
                              <Phone size={18} />
                            </Button>
                          )}
                          ...
                          <Sheet>
                            <SheetTrigger asChild>
                              <Button size="icon" className="rounded-full bg-[#00d4ff] text-black hover:bg-[#00d4ff]/80 relative">
                                ...
                                {activeRide.unread_messages > 0 && <span className="absolute -top-1 -right-1 bg-red-500 w-3 h-3 rounded-full border-2 border-black"></span>}
                              </Button>
                            </SheetTrigger>
                            <SheetContent side="bottom" className="h-[80vh] bg-black border-t border-[#00ff88]/30">
                              <ChatInterface rideId={activeRide.id} />
                            </SheetContent>
                          </Sheet>
                        </div>
                      </div>
                  ...
                  <div className="space-y-4 px-2 relative">
                    ...
                    <div className="flex gap-3 relative z-10">
                      ...
                      <div>
                        ...
                        <p className="text-white text-sm">{activeRide.destination || t("where_to")}</p>
                      </div>
                    </div>
                  </div>

                  {["searching", "accepted"].includes(activeRide.status) && (
                    <Button variant="ghost" className="w-full text-red-500 hover:text-red-400 hover:bg-red-500/10 mt-4" onClick={handleCancelRide}>
                      {t("cancel_ride")}
                    </Button>
                  )}
                </CardContent>
                  ...
                  <div className="bg-white p-2 rounded-lg">
                    <PayPalButtons
                      ...
                      createOrder={(data, actions) =>
                        actions.order.create({
                          purchase_units: [{ amount: { value: String(topupAmount), currency_code: "USD" } }],
                        })
                      }
                      onApprove={async (data, actions) => {
                        await actions.order.capture();
        ...
        <Dialog open={showRatingModal} onOpenChange={setShowRatingModal}>
          <DialogContent className="bg-[#1a1a2e] border border-[#00ff88]/20 text-white">
            ...
            />
            <Button onClick={submitRating} className="w-full bg-[#00ff88] text-black mt-4 font-bold">
              {t("submit_feedback")}
            </Button>
          </DialogContent>
        </Dialog>
