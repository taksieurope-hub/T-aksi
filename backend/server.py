...
# Firebase
SERVICE_ACCOUNT_PATH = Path(os.environ.get("FIREBASE_SERVICE_ACCOUNT_PATH", str(ROOT_DIR / "firebase-service-account.json")))
...
def serialize_firestore_data(data: dict) -> dict:
    ...
    for key, value in data.items():
        ...
        elif hasattr(value, "_sentinel"):  # SERVER_TIMESTAMP sentinel
            result[key] = datetime.now(timezone.utc).isoformat()
        elif isinstance(value, list):
            result[key] = [
                serialize_firestore_data(item) if isinstance(item, dict) else
...
class RideRequest(BaseModel):
    ...
    # PayPal order id (created client-side)
...
async def request_ride(ride_data: RideRequest, background_tasks: BackgroundTasks, user_id: str = Depends(get_current_user_id)):
    ...
    new_ride = {
        ...
        "payment_order_id": ride_data.payment_order_id,  # IMPORTANT: save it
...
async def accept_ride(ride_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    db.collection("rides").document(ride_id).update({
        ...
        "commission_paid": held_commission,  # store the hold
...
async def complete_ride(
    ...
    total_wait_minutes: Optional[int] = None,  # kept for compatibility even if not used
    user_id: str = Depends(get_current_user_id),
):
    db = get_db()
    ...
    # DRIVER EARNINGS / COMMISSION RECONCILIATION (fixes double charge)
    ...
    if driver_id:
        ...
        delta_commission = actual_commission - held_commission  # positive => driver owes more; negative => refund
        ...
        if payment_method == "cash":
            # driver collected cash outside the system; balance should only adjust by commission difference
            updates["earnings.balance"] = firestore.Increment(-delta_commission)
        elif payment_status == "paid":
            # platform received money; driver should receive net earnings
            # balance after accept was B - held_commission
            # final should be B + driver_earnings, so increment by (held_commission + driver_earnings)
