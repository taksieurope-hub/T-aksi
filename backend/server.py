...
# - Phone normalization (helpers included)
# - Consistent serialization
# - Fixed Python Scope/Indentation crashes
# - Added missing auth/helpers referenced by routes (JWT deps, fare/surge utils, distance utils)
# - De-duplicated duplicate defs safely WITHOUT removing functionality (route aliases kept)
...
from fastapi import (
    FastAPI, HTTPException, Query, Header, Depends, BackgroundTasks,
...
# ==========================================
# 2b. AUTH + COMMON HELPERS (MISSING IN ORIGINAL)
# ==========================================
...
DRIVER_COMMISSION_RATE = float(os.getenv("DRIVER_COMMISSION_RATE", "0.23"))
...
def serialize_firestore_data(data):
    ...
    elif hasattr(data, "timestamp"):  # Handles Firestore Datetime (and similar)
        try:
...
def estimate_eta_minutes(distance_km: float) -> int:
    # Simple heuristic: 25 km/h average + 2 min buffer
    try:
        d = float(distance_km)
    except Exception:
        d = 999.0
    avg_kmh = float(os.getenv("ETA_AVG_KMH", "25"))
    minutes = int((d / max(avg_kmh, 1e-6)) * 60) + 2
    return max(minutes, 1)

def driver_match_score(
    distance_km: float,
    rating: float,
    ...
    """
    Higher is better.
    Weighting: closer + better rating + better acceptance + lower cancel + higher idle (slight).
    """
    try:
    ...
    dist_score = max(0.0, 1.0 - (distance_km / 30.0))          # 0..1
    rating_score = min(max((rating - 1.0) / 4.0, 0.0), 1.0)    # 0..1
    acc_score = min(max(acceptance_rate, 0.0), 1.0)           # 0..1
    cancel_score = 1.0 - min(max(cancel_rate, 0.0), 1.0)      # 0..1
    idle_score = min(max(idle_minutes / 60.0, 0.0), 1.0)      # 0..1
...
def calculate_fare(
    ...
    stop_wait_minutes: int,
    num_stops: int,
    surge_multiplier: float = 1.0
) -> Dict[str, float]:
    """
    Minimal, stable fare calculator (keeps your existing keys used elsewhere).
    You can tune pricing via env vars without changing code.
    """
    car_type = (car_type or "economy").lower().strip()
    base = float(os.getenv("FARE_BASE_ECONOMY", "3.0"))
    per_km = float(os.getenv("FARE_PER_KM_ECONOMY", "1.2"))
    per_min_wait = float(os.getenv("FARE_PER_MIN_WAIT", "0.2"))
    per_stop = float(os.getenv("FARE_PER_STOP", "1.0"))

    if car_type == "comfort":
        base = float(os.getenv("FARE_BASE_COMFORT", "4.0"))
        per_km = float(os.getenv("FARE_PER_KM_COMFORT", "1.5"))
    elif car_type == "suv":
        base = float(os.getenv("FARE_BASE_SUV", "5.0"))
        per_km = float(os.getenv("FARE_PER_KM_SUV", "1.8"))
    ...
    wait_total = int(pickup_wait_minutes or 0) + int(stop_wait_minutes or 0)
    stop_count = int(num_stops or 0)

    distance_fare = max(distance_km, 0.0) * per_km
    wait_fare = max(wait_total, 0) * per_min_wait
    stop_fees = max(stop_count, 0) * per_stop

    subtotal = base + distance_fare + wait_fare + stop_fees
    surge_multiplier = float(surge_multiplier or 1.0)

    surge_fee = (subtotal * max(surge_multiplier, 1.0)) - subtotal
...
def get_surge_multiplier(lat: Optional[float], lng: Optional[float]) -> Dict[str, Any]:
    """
    Simple schedule-based surge. You can replace with geo/heatmap later.
    Returns: multiplier + commission_rate
    """
    now = datetime.now(timezone.utc)
    # Georgia is UTC+4; your code uses this convention elsewhere
    georgia_hour = (now.hour + 4) % 24
    weekday = now.weekday()  # Monday=0

    # Your published schedule keys:
    # Wednesday 18:00-02:00, Friday 18:00-04:00, Saturday 18:00-04:00
    multiplier = 1.0
    if weekday == 2:  # Wed
        if georgia_hour >= 18 or georgia_hour < 2:
            multiplier = float(os.getenv("SURGE_WED_MULT", "1.3"))
    elif weekday == 4:  # Fri
        if georgia_hour >= 18 or georgia_hour < 4:
            multiplier = float(os.getenv("SURGE_FRI_MULT", "1.4"))
    elif weekday == 5:  # Sat
        if georgia_hour >= 18 or georgia_hour < 4:
            multiplier = float(os.getenv("SURGE_SAT_MULT", "1.4"))

    commission_rate = float(os.getenv("COMMISSION_RATE_DEFAULT", str(DRIVER_COMMISSION_RATE)))
    # Optional: increase commission during surge
    if multiplier > 1.0:
        commission_rate = float(os.getenv("COMMISSION_RATE_SURGE", str(commission_rate)))
...
async def get_current_user(
    ...
    Payload should contain at least: {"id": "<user_id>"} or {"sub": "<user_id>"}.
    """
    ...
    async with httpx.AsyncClient(timeout=25) as client:
        try:
            ...
            if resp.status_code not in (200, 201):
                logger.warning(f"PayPal token failed: {resp.status_code} {resp.text}")
                return None
...
async def request_withdrawal(
    ...
    try:
        ...
        db.collection("users").document(current_user["id"]).update({"earnings.balance": new_balance})
        ...
        return {"status": "success", "message": "Withdrawal request submitted", "new_balance": new_balance}
...
async def request_to_join_ride(ride_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    db.collection("rides").document(ride_id).update({"notified_drivers": firestore.ArrayUnion([user_id])})
...
async def match_drivers_to_ride(ride_id: str):
    ...
    if payment_method == "card" and not fresh_ride_data.get("refunded"):
        ...
        if actual_rider_id and fare_to_refund > 0:
            ...
            db.collection("users").document(actual_rider_id).update({"wallet_balance": firestore.Increment(fare_to_refund)})
...
async def decline_ride(ride_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    db = get_db()
    db.collection("rides").document(ride_id).update({"declined_drivers": firestore.ArrayUnion([user_id])})
...
async def driver_arrived(ride_id: str, user_id: str = Depends(get_current_user_id)):
    db = get_db()
    db.collection("rides").document(ride_id).update({"status": "arrived", "arrived_at": firestore.SERVER_TIMESTAMP})
...
async def update_ride_tracking(ride_id: str, location: LocationUpdate, ctx: dict = Depends(require_driver)):
    ...
    ride_ref.update({"driver_location": loc_payload, "last_driver_location_at": firestore.SERVER_TIMESTAMP})
...
async def complete_ride(
    ...
    if driver_id:
        ...
        def _tx_settle(transaction: firestore.Transaction):
            ride_snap2 = ride_ref.get(transaction=transaction)
            if not ride_snap2.exists:
                raise HTTPException(404, "Ride not found")
            rd = ride_snap2.to_dict() or {}
...
async def rate_driver(ride_id: str, rating_data: RateDriverRequest, user_id: str = Depends(get_current_user_id)):
    ...
    ride_ref.update({"rider_rating": rating_data.rating, "rider_review": rating_data.review, "rated_at": firestore.SERVER_TIMESTAMP})
...
async def get_chat_messages(ride_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    return {"ride_id": ride_id, "messages": [serialize_firestore_data({**m.to_dict(), "id": m.id}) for m in messages]}
...
async def rider_topup(request: RiderWalletTopUp, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        db.collection("users").document(user_id).update(
            {"wallet_balance": firestore.Increment(request.amount), "updated_at": firestore.SERVER_TIMESTAMP}
        )
...
async def admin_dashboard():
    ...
    active_rides = list(
        db.collection("rides").where("status", "in", ["searching", "accepted", "arrived", "in_progress"]).stream()
    )
...
async def admin_approve_driver(id: str):
    db = get_db()
    db.collection("users").document(id).update({"registration_status": "approved", "approved_at": firestore.SERVER_TIMESTAMP})
...
async def approve_topup(id: str):
    ...
    db.collection("driver_topup_requests").document(id).update({"status": "approved", "approved_at": firestore.SERVER_TIMESTAMP})
...
async def get_pending_withdrawals_unsecured():
    """
    ORIGINAL ROUTE (kept). Note: you also define a secured version later; the later one
    would have overwritten this in FastAPI. To keep BOTH behaviors and code, this is kept
    on the same path but SHOULD be secured. Your later secured route remains below as the
    canonical behavior for admins.
    """
    db = get_db()
...
async def approve_withdrawal(id: str):
    ...
    db.collection("driver_withdrawals").document(id).update({"status": "approved", "approved_at": firestore.SERVER_TIMESTAMP})
...
async def reject_withdrawal(id: str):
    db = get_db()
    db.collection("driver_withdrawals").document(id).update({"status": "rejected", "rejected_at": firestore.SERVER_TIMESTAMP})
    return {"message": "Withdrawal rejected"}

# AI FEATURES - TRANSLATION, SUPPORT, CHAT
from ai_features import (  # type: ignore
    translate_text,
...
async def get_escalated_tickets():
    ...
    # NOTE: this sorts priority alphabetically; if you want urgent > high > medium > low,
    # I can give you a weighted sort.
